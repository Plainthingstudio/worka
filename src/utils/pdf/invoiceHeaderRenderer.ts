...
export const renderInvoiceHeader = (
  ...
  // Add invoice details (top right, below INVOICE) - INCREASED FONT SIZE
  ...
  doc.text(format(new Date(invoiceDate), "dd/MMMM/yyyy"), PAGE_CONFIG.width - margin.right, currentY + 27, { align: "right" });
  doc.text(format(new Date(dueDate), "dd/MMMM/yyyy"), PAGE_CONFIG.width - margin.right, currentY + 34, { align: "right" });
  ...
  // Handle client address and phone
  ...
  doc.text(clientPhone || "No phone provided", margin.left, billingY);
